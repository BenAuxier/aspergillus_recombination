...
xTickMarks = tapply(alignments$refEnd2, alignments$refID, max)
yLabelPos = tapply(alignments$queryEnd2, alignments$queryID, mean)
xLabelPos = tapply(alignments$refEnd2, alignments$refID, mean)
options(warn = -1) # turn off warnings
gp <- ggplot(alignments) +
    geom_hline(yintercept = yTickMarks,color = "grey70",size = 0.1) +
    geom_vline(xintercept = xTickMarks,color = "grey70",size = 0.1) +
    #geom_point(mapping = aes(x = refStart2, y = queryStart2),size = 0.02) +
    #geom_point(mapping = aes(x = refEnd2, y = queryEnd2),size = 0.02) +
    geom_segment(aes(x = refStart2,xend = refEnd2,y = queryStart2,yend = queryEnd2)) +
    scale_x_continuous(breaks = xLabelPos,
                       expand=c(0,0),
    ...
    scale_y_continuous(breaks = yLabelPos, labels = substr(levels(alignments$queryID), start = 1, stop = 20),expand=c(0,0)) +
...
ggsave(filename = paste0(opt$output_filename, ".pdf"), width = opt$plot_size, height = opt$plot_size, units = "in", limitsize = F)
